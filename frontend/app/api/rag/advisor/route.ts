...
const BACKEND_TIMEOUT_MS = Number(process.env.BACKEND_TIMEOUT_MS || '200000');
...
export async function POST(request: NextRequest) {
  ...
  try {
    const response = await fetch(`${backendUrl}/rag/advisor`, {
      method: 'POST',
    ...
    if (!payload) {
      return NextResponse.json(
        { detail: 'Backend returned an invalid response payload' },
        { status: 502 }
      );
    }
    ...
    return NextResponse.json(
      {
        detail: isTimeout ? 'Advisor request timed out' : 'Internal server error',
      },
